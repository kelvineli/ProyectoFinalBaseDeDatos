...
import { tickets, tecnicos, obtenerTicket, actualizarTecnicoTicket } from "./sqlConexion";
...
export default function ModalEstado({ visible, idTicket, onClose, onGuardado }) {
    ...
    if(visible){
        ...
        const cargarDatosTecnicos = async () => {
            ...
            if (!result.success) {
                ...
                const tecnicosListados = (result.data || []).map((p) => ({
                    value: p.idUsuario,
                    label: `${p.Nombre}`,
                }));
                setEstadosList(tecnicosListados);
            }
        };
  ...
  const handleGuardar = async (crear = true) => {
    if(crear){
      const result = await actualizarTecnicoTicket({ idTicket, ...formData });
  ...
  return (
    <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-[#58585a] text-white rounded-2xl w-full max-w-2xl shadow-lg relative max-h-[90vh] overflow-y-auto p-8 scrollbar-thin">
        <h2 className="text-2xl text-center mb-6">ASIGNAR TÉCNICO AL TICKET</h2>

        {/* Formulario */}

          {/* Categoría */}
            <div>
              <label className="block uppercase text-sm mb-1">Técnico:</label>
              <Select
                options={estadosList}
